...
\begin{frame}[fragile]
	...
	\begin{figure}[ht]
		\centering
		\includegraphics[width=0.25\textwidth]{figs/bombe-loops.png}
		\caption*{The letters of a crib and ciphertext expressed as a graph to provide a menu which specifies how to set up a bombe run.}
	\end{figure}
\end{frame}

\note{
	Aspects of numerical technique in the Bombe:
	\begin{itemize}
		\item Initial guess (the better the guess, the better is the solution)
		\item Error quantifying
		\item Algorithm
		\item Identify divergence from a solution
		\item Verification
	\end{itemize}
}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\mode<beamer>{
			...
			programs can do to them. For example matrices.
		}
		\mode<handout>{
			\vspace{2cm}
		}
